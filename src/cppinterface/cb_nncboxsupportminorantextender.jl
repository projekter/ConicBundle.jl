...
"""
CBNNCBoxSupportMinorantExtender(fun::Union{<:CBNNCBoxSupportFunction,Nothing}) = CBNNCBoxSupportMinorantExtender(@ccall libcb.cb_nncboxsupportminorantextender_new((isnothing(fun) ? C_NULL : fun.data)::Ptr{Cvoid})::Ptr{Cvoid})
...
"""
cb_extend!(self::CBNNCBoxSupportMinorantExtender, minorant::CBMinorant, n_coords::Integer, indices::Union{<:AbstractVector{Integer},Nothing}) = GC.@preserve indices begin
    (LinearAlgebra.chkstride1(indices); @ccall libcb.cb_nncboxsupportminorantextender_extend(self.data::Ptr{Cvoid}, minorant.data::Ptr{Cvoid}, n_coords::Cint, indices::Ptr{Cint})::Cint)
end
