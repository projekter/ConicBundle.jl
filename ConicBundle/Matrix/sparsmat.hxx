...
/// returns C=beta*C+alpha* A*D*A^T, where D is a vector representing a diagonal matrix and A may be transposed; if beta==0. then C is initialized to the correct size
    friend Symmatrix& scaledrankadd(const Sparsemat& A, const Matrix& D, Symmatrix& C,
      Real alpha, Real beta, int trans);
    // returns C=beta*C+alpha* A*D*A^T, where A may be transposed
...
///
    ...
    friend inline Real norm2(const Sparsemat& A);               //=sqrt(ip(A,A));
  ...
  inline void Sparsemat::init_to_zero() {
    ...
    nr = nc = 0;
    tol = SPARSE_ZERO_TOL;
    colinfo.init(0, 3, Integer(0)); colindex.init(0, 1, Integer(0)); colval.init(0, 1, 0.);
    rowinfo.init(0, 3, Integer(0)); rowindex.init(0, 1, Integer(0)); rowval.init(0, 1, 0.);
#if (CONICBUNDLE_DEBUG>=1)
    is_init = true;
#endif
  }
  ...
  ///returns the Frobenius norm of A, i.e., the square root of the sum of A(i,j)*A(i,j) over all i,j 
  inline Real norm2(const Sparsemat& A) {
    chk_init(A);
    return norm2(A.colval);
  }
