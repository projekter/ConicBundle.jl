...
namespace CH_Matrix_Classes {
  ...
  Sparsesym& Sparsesym::support_xbpeya(const Sparsesym& B, Real alpha, Real beta) {
    chk_add(*this, B);
    if (beta != 1.)
      colval *= beta;
    if (alpha != 0.) {
      Integer Bcr = 0;
      for (Integer cr = 0; cr < colinfo.rowdim(); cr++) {
        Integer colind = colinfo(cr, 0);
        while ((Bcr < B.colinfo.rowdim()) && (B.colinfo(Bcr, 0) < colind))
          Bcr++;
        Integer Bcolind = nr;
        if (Bcr < B.colinfo.rowdim())
          Bcolind = B.colinfo(Bcr, 0);
        if (Bcolind != colind)
          continue;
        const Integer* Bind = B.colindex.get_store() + B.colinfo(Bcr, 2);
        const Integer* const Bindend = Bind + B.colinfo(Bcr, 1);
        const Real* Bval = B.colval.get_store() + B.colinfo(Bcr, 2);
        const Integer* ind = colindex.get_store() + colinfo(cr, 2);
        const Integer* const indend = ind + colinfo(Bcr, 1);
        Real* val = colval.get_store() + colinfo(cr, 2);
        if ((Bind != Bindend) && (ind != indend)) {
          do {
            if (*ind == *Bind) {
              (*val++) += alpha * (*Bval++);
              if (++ind == indend)
  ...
  Matrix& genmult(const Sparsesym& A, const Matrix& B, Matrix& C,
    ...
    if (btrans) {
      ...
      const Real* vp = A.colval.get_store();
  ...
  Matrix& genmult(const Matrix& A, const Sparsesym& B, Matrix& C,
    ...
    if (atrans) {
      ...
      const Integer* ip = B.colindex.get_store();
      const Real* vp = B.colval.get_store();
      const Real* abp = A.m;
      Integer anr = A.nr;
      Real* cbp = C.get_store();
