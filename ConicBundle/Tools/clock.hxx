...
namespace CH_Tools {
  ...
  */
  ...
  public:
    ...
    Microseconds() {
      seconds = 0; microsecs = 0; infinity = false;
    }
    /// constructor for setting value to 0 or infinity
    Microseconds(bool infty) {
      seconds = 0; microsecs = 0; infinity = infty;
    }
    /// copy constructor
  ...
  class Clock {
  private:
    Microseconds t_start; ///< user time value on intialzation
    Microseconds offset; ///< use specified offset 
    Microseconds wall_start; ///< wall clock time on initilazation
#ifdef __unix
#else
    long l_start; ///< if not unix, use long values instead
#endif
