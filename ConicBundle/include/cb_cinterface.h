...
/**@defgroup cinterface Interface to ConicBundle for the Language C
   ...
   Internal details are sketched in \ref internal_cinterface.
...
extern "C" {
   ...
   * ***************************************************************** */
   ...
   /** @brief  Initializes the problem by setting the design space (the dimension and possible box constraints of the variables)
     ...
     by cb_set_lower_bound() or cb_set_upper_bound().
        ...
        @param[in] lowerb  (either pointer to double array of length @a m or NULL)
        ...
        @param[in] upperb  (either pointer to double array of length @a m or NULL)
   ...
   * ***************************************************************** */
   ...
   /** @brief Adds the function, the sum of which should be minimized, to the problem description.
      ...
      @param[in] primaldim  (int)
          May be zero, otherwise in each call to @a f enough storage will
          be provided to store a primal generating vector for each  subgradient
   ...
   * ***************************************************************** */
   ...
   /** @brief solves or does a prescribed number of

     Bundle methods solve a problem by a sequence of so called descent
     steps that actually bring progress by moving from the current
     "center point" to a new center with better objective.  A descent
     step may consist of several function evaluations (null steps), that
     lead to no immediate progress but mainly improve a cutting model of
     the objective function close to the current center point.  A
     minimizer to the model is accepted as descent step if the function
     value at this point satisfies a sufficient decrease criterion in
     comparison to the decrease predicted by the model. Having found a
     descent step, the next center is automatically shifted to this
     successful candidate.  Termination criteria may stop the process of
     seeking for a descent step, in which case the current center is kept
     and the routine termination_code() returns the termination code.
     ...
     Therefore, a descent step is the smallest unit, after which user
     interaction can take place safely. To allow this there is a flag
     stop_at_descent_steps that will cause the code to return after the
     next descent step.

     If you know what your are doing, you may also use the input
     parameter maxsteps to force the algorithm to return after at most
     maxsteps null steps. Calling solve again without any intermediate
     problem configurations will then simply continue the process where
     it stopped and convergence is save. During null steps one may not
     decrease the weight or delete nonzero variables of the center or the
     current candidate!

     In a Lagrangean relaxation cutting plane approach one may want to
     separate and enlarge the dimension after a certain number of null
     steps. In this case the code will try to preserve the model, given
     appropriate subgradient extension routines have been provided. If
     the model cannot be extended, it has to be discarded (if subgradient
     extension is not successful this is done automatically), and the
     algorithm will be restarted from the current center point.

     @param[in] p  (cb_problemp)
         pointer to the problem

     @param[in] maxsteps (int)
         use value =0 as default (anything <= serves as infinite upper bound),
         if maxsteps>0 the code returns after at most so many null steps

     @param[in] stop_at_descent_steps (int)
         if true(!=0) the code also returns whenever a descent step occured,
         if false(==0) it only stops after maxsteps or when a termination
         criterion is met
   ...
   * ***************************************************************** */
   ...
   /** @brief Returns the current approximate primal solution for the function
      ...
      Likewise, the routine is meaningful only if @a primaldim was set in
   ...
   * ***************************************************************** */
   ...
   /** @brief Sets the maximum number of subgradients stored for use in forming
     the model or determining metric information of the function having
     this @a function_key. it must be as least as large as max_modelsize
     (and is increased to this if not)
   ...
   * ***************************************************************** */
   ...
   /** @brief Retrieves the two bundle parameters specified in the
        routines cb_set_max_modelsize() and cb_set_max_bundlesize().
      ...
      @param[out] max_modelsize (pointer to int)
          returns the maximum number of subgradients to be used in forming the cutting model

      @param[out] max_bundelsize (pointer to int)
          returns maximum number of subgradients stored for use in formint the cutting model
  ...
  */
   ...
   * ***************************************************************** */
   ...
   /** @brief Use a variable metric heuristic or switch it off alltogether.
        (the variable metric heuristic resets the quadratic term to some diagonal
        or low rank matrix, switching it off resets the proximal term to the
        identity)
   ...
   * ***************************************************************** */
   ...
   /** @brief If set to true (the default is false),
       ...
       can be obtained via the routine cb_get_fixed_active_bounds().
