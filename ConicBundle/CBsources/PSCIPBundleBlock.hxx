/* ****************************************************************************
    ...
    File:  CBsources/PSCIPBundleBlock.hxx
...
#ifndef CONICBUNDLE_PSCIPBUNDLEBLOCK_HXX
#define CONICBUNDLE_PSCIPBUNDLEBLOCK_HXX

/**  @file PSCIPBundleBlock.hxx
    @brief Header declaring the class ConicBundle::PSCIPBundleBlock
    @version 1.0
    @date 2020-05-02
...
namespace ConicBundle {
   ...
   */
   ...
   /** @brief defines an interface for passing on oracle information to
       PSCIPBundleBlock

   */

  class QPPSCOracleDataObject : public QPModelOracleDataObject {
  public:
    virtual ~QPPSCOracleDataObject();
  ...
  /** @brief  interior point variables and routines specific to the primal dual complementarity conditions of a positive semidefinite cone with special routines for handling the bundle and the trace constraint

      adds bundle functionality to PSCIPBlock
  */

  class PSCIPBundleBlock : public InteriorPointBundleBlock, public PSCIPBlock {
  private:
    CH_Matrix_Classes::Matrix B;       ///< matrix representation of bundle
    ...
    CH_Matrix_Classes::Indexmatrix Bnzcolind; ///< holds the indices of nonzero columns of B in increasing order

    QPPSCOracleDataObject* oracle_data; ///< if not NULL this makes the data of the underlying psc oracle accessible; it is yet to be explored whether this can help 

    CH_Matrix_Classes::Matrix testmat; ///< for comparisons in testing


    /// try to determine the number of active eigenvalues / the dimension of the active eigenspace of X
    CH_Matrix_Classes::Integer active_rank(const CH_Matrix_Classes::Matrix& lamX,
    ...
    int get_growth(const CH_Matrix_Classes::Matrix& lamX,
      const CH_Matrix_Classes::Matrix& vecX,
    ...
    int form_B(CH_Matrix_Classes::Integer dim,
    ...
    int sveciB_times_vec(const CH_Matrix_Classes::Matrix& vec,
      CH_Matrix_Classes::Matrix& Bvec);
  ...
  public:
    ...
    virtual void set_oracle_data(QPModelOracleDataObject* in_oracle_data) {
      oracle_data = dynamic_cast<QPPSCOracleDataObject*>(in_oracle_data);
    }
    ...
    */
    ...
    int get_pscx(CH_Matrix_Classes::Matrix& pscx_eigs,
