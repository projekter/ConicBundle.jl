...
#ifndef CONICBUNDLE_BOXMODEL_HXX
#define CONICBUNDLE_BOXMODEL_HXX

/**  @file BoxModel.hxx
    @brief Header declaring the class ConicBundle::BoxModel
    @version 1.0
    @date 2014-07-23
...
/**@brief implements (as a derived class of ConeModel) a standard cutting plane model for a MatrixBoxOracle 
 */


class BoxModel: public ConeModel
{
private:
  ...
  int adapt_bundle_to_parameters(CH_Matrix_Classes::Integer cand_id,
    ...
    /// returns true if clear_aggregates() also clears all minorant information and this does not harm the rest of the model during modifications (i.e., not routine for extending/adapting minorants is required) 
 ...
 virtual int start_local_augmodel(QPModelDataPointer& qpblockp,
  ...
  {return (data.cand_ub_mid==data.get_modification_id())&&(data.cand_minorant.valid()); }
...
public:    
  ...
  // as in SumBlockModle::update_model(ModelUpdate,CH_Matrix_Classes::Integer,const CH_Matrix_Classes::Matrix&,CH_Matrix_Classes::Real,CH_Matrix_Classes::Real,CH_Matrix_Classes::Real&,CH_Matrix_Classes::Real&)
