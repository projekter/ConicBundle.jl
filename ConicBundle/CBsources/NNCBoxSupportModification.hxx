/* ****************************************************************************
    ...
    File:  CBsources/NNCBoxSupportModification.hxx
...
#ifndef CONICBUNDLE_NNCBOXSUPPORTMODIFICATION_HXX
#define CONICBUNDLE_NNCBOXSUPPORTMODIFICATION_HXX


/**  @file NNCBoxSupportModification.hxx
    @brief Header declaring the class ConicBundle::NNCBoxSupportModification
    @version 1.0
    @date 2014-11-10
...
/** @ingroup implemented_matrix_function_oracle
*/
//@{

/** @brief Collects modifications for NNCBoxSupport for appending, deleting or reassigning variables and respective upper and lower bounds

    This class is implemented by restricting the possibilities of ConicBundle::Modification to the operations involving variables, so all routines simply call those of Modification. See the description there for usage and default values. The default value is 0 for lower or upper bounds that are not given explicitly.

 */


  class NNCBoxSupportModification: public CBout, public OracleModification
{
protected:
...
public:
  ...
  NNCBoxSupportModification(CH_Matrix_Classes::Integer var_olddim=0,
			 const CBout* cb=0,
			 int incr=-1):
  ...
  virtual ~NNCBoxSupportModification();
  ...
  {return mdf.add_append_vars(append_dim,append_lb,append_ub,0,0,0);}
 ...
 /// returns true if all entries deleted in @a oldpoint (must be a vector of length old_vardim()) are 0,  false otherwise
  ...
  {
    const NNCBoxSupportModification* gm=dynamic_cast<const NNCBoxSupportModification*>(&m);
    if (gm) {return mdf.incorporate(gm->mdf);} 
    return 1;
  }
  ...
  {return new NNCBoxSupportModification(old_var_dim);}
