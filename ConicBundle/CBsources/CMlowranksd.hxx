/* ****************************************************************************
    ...
    File:  CBsources/CMlowranksd.hxx
...
#ifndef CONICBUNDLE_CMLOWRANKSD_HXX
#define CONICBUNDLE_CMLOWRANKSD_HXX

/**  @file CMlowranksd.hxx
    @brief Header declaring the class ConicBundle::CMlowranksd (needed for ConicBundle::AffineMatrixFunction)
    @version 1.0
    @date 2015-03-24
...
//@{
 ...
 /**@brief implements a low rank matrix \f$AB^T+BA^T\f$ as Coeffmat with \f$A\f$ a sparse rectangular CH_Matrix_Classes::Sparsemat and \f$B\f$ a dense rectangular CH_Matrix_Classes::Matrix (for use with MatrixSDPfunction, see \ref implemented_psc_oracle).
  */

  class CMlowranksd: public Coeffmat
  {
  private:
  ...
  public:
    ...
    {A=Ain;B=Bin;CM_type=CM_lowranksd;infop=cip;}
    ///
    virtual ~CMlowranksd(){}
    ...
    {return new CMlowranksd(A,B,ConicBundle::clone(infop));}
    ...
    { CH_Matrix_Classes::Matrix C,D; CH_Matrix_Classes::genmult(A,B,C,1.,0.,1); CH_Matrix_Classes::genmult(C,C,D);
	...
	for(CH_Matrix_Classes::Integer i=colinfo(j,1);--i>=0;){
      ...
      CH_Matrix_Classes::Real* dp=D.get_store();
	...
	for(CH_Matrix_Classes::Integer i=D.rowdim();--i>=0;){
	  (*dp++)=CH_Matrix_Classes::mat_ip(ard,ap,pp);
	  ap+=ard;
	}
      }}	
      chk_set_init(D,1);
    ...
    { return 4*A.nonzeros()+4*B.rowdim()*B.coldim(); }
    ...
    {return 2.*CH_Matrix_Classes::ip(S*A,B);}
    ...
    {
      const CMlowranksd *pp=dynamic_cast<const CMlowranksd *>(p);
      if (pp==0) 
	return 0;
      if (CH_Matrix_Classes::equal(A,pp->A,tol) &&
	  (B.rowdim()==(pp->B).rowdim())&&(B.coldim()==(pp->B).coldim()) &&
	  (CH_Matrix_Classes::norm2(B-pp->B)<tol))
	return 1;
      return 0;
    }
    ...
    {return i>>A>>B;     
      if((A.rowdim()!=B.rowdim())||(A.coldim()!=B.coldim())){
         ...
         if (CH_Matrix_Classes::materrout) (*CH_Matrix_Classes::materrout)<<"*** ERROR: CMlowranksd::in(): dimensions of A and B do not match"<<std::endl;
      }
      return i;
    }
    ...
    {CM_type=CM_lowranksd;infop=cip;in(is);}
