...
namespace ConicBundle {
  ...
  public:
    ...
    void set_exacteigs(Integer i) {
      exacteigs = max(i, Integer(0));
    }

    void set_dense_limit(Integer lim) {
      dense_limit = max(lim, Integer(0));
    }

    const Bigmatrix& get_bigmat() const {
      return bigmat;
    }
    ...
    */
        ...
        */
        ...
        if (cb_out(0)) {
          if (nreig > 0) {
            get_out() << " eigmax="; get_out().precision(8); get_out() << Ritz_values(0);
          } else {
            get_out() << " Ritz_val="; get_out().precision(8); get_out() << Ritz_values(0);
          }
