/* ****************************************************************************
    ...
    File:  CBsources/NNCIPBundleBlock.hxx
...
#ifndef CONICBUNDLE_NNCIPBUNDLEBLOCK_HXX
#define CONICBUNDLE_NNCIPBUNDLEBLOCK_HXX

/**  @file NNCIPBundleBlock.hxx
    @brief Header declaring the class ConicBundle::NNCIPBundleBlock
    @version 1.0
    @date 2020-05-02
...
namespace ConicBundle {
   ...
   */
   ...
   /** @brief  interior point variables and routines specific to primal dual complementarity conditions of a nonnegative cone with special routines for handling the bundle and the trace constraint

       adds bundle functionality to NNCIPBlock
   */

  class NNCIPBundleBlock : public InteriorPointBundleBlock, public NNCIPBlock {
  private:
    ...
    /// this is currently not in use but was used for experiments with eliminating some coordinates on the fly; did not work
  ...
  public:
    ...
    // 		       CH_Matrix_Classes::Matrix* nncx_activity=0) const;
    ...
    */
    ...
    /// after the bundle subproblem is solved, this retrieves the local linear solution vector; if nncx_activity is set, the values between zero and one indicate the guess on the coefficients activity level 
    virtual int get_nncx(CH_Matrix_Classes::Matrix& nncx,
      CH_Matrix_Classes::Matrix* nncx_activity,
      CH_Matrix_Classes::Real trace_rhs,
      bool cautious) const;
